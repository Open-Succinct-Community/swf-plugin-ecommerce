...
public interface MarketPlaceSkuUpdateQueue extends Model {
    static void push(Inventory inventory) {
        if (inventory.getFacility().getPreferredMarketPlaceIntegrations().isEmpty()){
            return;
        }
        List<MarketPlaceSkuUpdateQueue> queueRecords = entries(inventory.getSkuId(),inventory.getFacilityId());

        if (queueRecords.isEmpty()){
            MarketPlaceSkuUpdateQueue entry = Database.getTable(MarketPlaceSkuUpdateQueue.class).newRecord();
            entry.setSkuId(inventory.getSkuId());
            entry.setFacilityId(inventory.getFacilityId());
            entry.save();
        }
    }
    static List<MarketPlaceSkuUpdateQueue> entries(long skuId, long facilityId){
        Select select = new Select().from(MarketPlaceSkuUpdateQueue.class);
